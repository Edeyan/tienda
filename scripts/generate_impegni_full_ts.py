import re
import json

with open('./scripts/parse_impegni.js', 'r', encoding='utf-8') as f:
    content = f.read()

m = re.search(r'const rawText = `([\s\S]*?)`;', content)
raw = m.group(1)

pages_raw = re.split(r'Total de Artículos: 270\s*\n\s*Page \d+ of 31', raw)

all_parsed_products = []

for page_idx in range(len(pages_raw)):
    page_text = pages_raw[page_idx].replace('\x0c', '').replace('FERRETERIA', '').strip()
    if not page_text:
        continue
    
    # We want to extract each item
    codes = re.findall(r'\b(1[23]\d{3})\b', page_text)
    models_raw = re.findall(r'\b([A-Z]{2,3}\d{4,5}[A-Z0-9]*)\b', page_text)
    models = [mod for mod in models_raw if mod not in ['IMPEGNI', 'FERRETERIA', 'KG', 'PZA', 'CBM', 'SET', 'USD', 'INV']]
    prices_inv = re.findall(r'\$(\d+(?:\.\d+)?)\s+Inv:\s*(\d+)', page_text)
    if len(prices_inv) < len(codes):
        prices_only = re.findall(r'\$(\d+(?:\.\d+)?)', page_text)
        invs_only = re.findall(r'Inv:\s*(\d+)', page_text)
        prices_inv = list(zip(prices_only, invs_only))
        
    cbms = re.findall(r'CBM:\s*(\d+\.\d+)', page_text)
    pesos = re.findall(r'(\d+(?:\.\d+)?)\s*KG', page_text)
    pie_cubs = re.findall(r'Pie/Cub:\s*(\d+\.\d+)', page_text)
    empaques = re.findall(r'Empaque:\s*(\d+)', page_text)
    
    # We will build products
    for i in range(len(codes)):
        code = codes[i]
        model = models[i] if i < len(models) else ""
        price = float(prices_inv[i][0]) if i < len(prices_inv) else 0.0
        stock = int(prices_inv[i][1]) if i < len(prices_inv) else 0
        peso = pesos[i] + " KG" if i < len(pesos) else ""
        cbm = cbms[i] if i < len(cbms) else ""
        pie_cub = pie_cubs[i] if i < len(pie_cubs) else ""
        empaque = empaques[i] if i < len(empaques) else ""
        
        all_parsed_products.append({
            'code': code,
            'model': model,
            'price': price,
            'stock': stock,
            'peso': peso,
            'cbm': cbm,
            'pie_cub': pie_cub,
            'empaque': empaque,
            'page': page_idx + 1
        })

print(f"Parsed {len(all_parsed_products)} products successfully.")
