...
with open('scripts/part3_products.json', 'r', encoding='utf-8') as f:
    new_products = json.load(f)

print(f"Loaded {len(new_products)} products from scripts/part3_products.json")
...
for p in new_products:
    ...
    # standardize specs keys
    std_specs = {
        "CÓDIGO": code,
        "MODELO": p['sku'],
        "EMPAQUE": str(p['specs'].get('Empaque', '')).replace(' PZA', '').replace(' ROLL', '').replace(' CJ', '').replace(' SET', '').replace(' PAR', '').replace(' MTR', '').replace(' METRO', '').replace(' BLISTER', '').replace(' BOT', '').strip(),
        "PESO": str(p['specs'].get('Peso', '')),
        "PIE/CUB": str(p['specs'].get('Pie/Cub', '0.0000')),
        "CBM": str(p['specs'].get('CBM', '0.000')),
        "INVENTARIO": f"{p['stock']} {p['specs'].get('Unidad', 'PZA')}"
    }
    p['specs'] = std_specs
    p['description'] = f"{p['name']}. Modelo: {p['sku']}, Código: {code}. Empaque: {std_specs['EMPAQUE']}, Peso: {std_specs['PESO']}, CBM: {std_specs['CBM']}. Stock disponible: {std_specs['INVENTARIO']}."
